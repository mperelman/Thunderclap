...
from lib.index_builder import build_indices, build_endnote_mappings, save_indices, split_into_chunks, augment_indices_with_identities
    ...
    try:
        ...
        if os.path.exists(identity_file):
            ...
            for identity, data in identity_data['identities'].items():
                ...
                # Add to index (merge with existing if identity term already exists)
                if identity_lower in indices['term_to_chunks']:
                    existing = set(indices['term_to_chunks'][identity_lower])
                    for chunk_id in chunk_ids_str:
                        if chunk_id not in existing:
                            indices['term_to_chunks'][identity_lower].append(chunk_id)
                            augmentation_count += 1
                else:
                    indices['term_to_chunks'][identity_lower] = chunk_ids_str
