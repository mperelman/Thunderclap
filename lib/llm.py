...
class LLMAnswerGenerator:
    ...
    def __init__(self, api_key=None, key_manager=None):
        ...
        self._executor = None  # Used when key_manager is set: non-blocking client creation + timeout
        if key_manager:
            # Use executor: no client in __init__, no key consumed; executor gets key and client per request
            # (run_in_executor for get_llm_client so list_models doesn't block; wait_for on API call)
            from lib.llm_executor import LLMExecutor
            from lib.llm_config import get_llm_client
            def _client_factory(key_str, model_index=0):
                return get_llm_client(api_key=key_str, model_index=model_index)
            _max_total = os.environ.get("LLM_MAX_TOTAL_TIME", "300") or "300"
            try:
                max_total_time = float(_max_total)
    ...
    async def call_api_async(self, prompt: str) -> str:
        ...
        Raises:
            Exception: If API call fails and no fallback available
        """
        # When using key manager, use executor (run_in_executor for client, wait_for for API, PINPOINT)
        if self._executor:
            return await self._executor.execute_async(prompt)
        ...
        while attempts < max_attempts:
            ...
            try:
                ...
                try:
                    ...
                    # No valid Part (finish_reason=1 with no content) -> return "" so caller can fall back instead of crashing
                    if "Part" in error_str or "part" in error_str.lower() or "valid" in error_str.lower():
                        return ""
                    # Re-raise original exception if it's not finish_reason/Part related
                    raise
            except Exception as e:
                last_err = e
                error_msg = str(e)
                    ...
                    if self.key_manager and current_key:
                        ...
                        if next_key and next_key != current_key:
                            ...
                            if available == 0:
                                ...
                                if total == 1:
                                    raise Exception(
                                        f"API key was reported as leaked. Only 1 key was found in Railway environment variables.\n\n"
                                        f"Please add multiple keys to Railway:\n"
                                        f"1. GEMINI_API_KEY = (new key)\n"
                                        f"2. GEMINI_API_KEY_1 = (new key)\n"
                                        f"3. GEMINI_API_KEY_2 = (new key)\n"
                                    ...
                                    raise Exception(f"All {total} API keys are leaked or invalid. Please add new keys to Railway environment variables (GEMINI_API_KEY, GEMINI_API_KEY_1, etc.).")
                            else:
                                # Wait a bit and try again with next key
                                await asyncio.sleep(1)
                                attempts += 1
